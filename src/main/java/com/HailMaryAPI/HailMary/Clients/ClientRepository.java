...
public interface ClientRepository extends JpaRepository<Client, Integer> {
    ...
    @Query("UPDATE Client SET email = ?1, first_name = ?2, last_name = ?3, date_of_birth = ?4, phone_number = ?5, " +
            "street_address = ?6, province = ?7, country = ?8, postal_code = ?9, password = ?10 " +
            "WHERE client_id = ?11")
    Optional<Client> updateClient(String email, String first_name, String last_name, String date_of_birth,
